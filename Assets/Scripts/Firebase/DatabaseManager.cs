...
public class DatabaseManager : MonoBehaviour
{
    ...
    private List<EventSource> activeListeners = new List<EventSource>();
    ...
    public void GetShallowTest(string path, Action<Dictionary<string, bool>> onGet, Action<Exception> onFailed = null)
    {
        //string asd = "shallow=true&print=pretty"
        string url = root + "/" + path + ".json";

        Dictionary<string, object> parameters = new Dictionary<string, object>();
        if (FirebaseProjectConfigurations.PROJECT_BUILD == ProjectBuildType.Emulator) parameters.Add("ns", FirebaseProjectConfigurations.PROJECT_ID);
        if (FirebaseManager.Instance.Auth.IdToken != null) parameters.Add("auth", FirebaseManager.Instance.Auth.IdToken);
        ...
        }).Then(response =>
        {
            Debug.Log($"[Database manager] Json data was get by url - {url}");
            string jsonResponse = System.Text.Encoding.UTF8.GetString(response.Data);
            JSONNode jsonObj = JSON.Parse(jsonResponse);
            if (jsonObj == null)
            {
                onGet(null);
                return;
            ...
            else
            {
                ...
                {
                    result.Add(child.Key.ToString(), child.Value.AsBool);
                }
    ...
    /* TRANSACTION */
    public void MakeTransaction(string path, string value)
    {
    ...
    /* LISTEN */
    public EventSource ListenForValueChanged(string path, Action<string> onNewMessage, Action<Exception> onError = null, bool openImmediately = true)
    {
        string url = BuildUrl(path);
        ...
        listener.OnOpen += (eventSource) => { Debug.Log($"Listener by path {path} opened! Uri - {listener.Uri}, Connection key - {eventSource.ConnectionKey}"); };
    ...
    public EventSource ListenForChildChanged(string path, OnEventDelegate handler, Action<Exception> onError = null, bool openImmediately = true)
    {
        string url = BuildUrl(path);
...
#if UNITY_WEBGL && !UNITY_EDITOR
        listener.On("patch", handler);
        listener.On("put", handler);
#elif UNITY_EDITOR || UNITY_STANDALONE
        listener.On("put", handler);
        listener.On("patch", handler);
#endif
    ...
    public EventSource ListenForChildChanged(string path, Action<string> onNewMessage, Action<Exception> onError = null, bool openImmediately = true)
    {
        string url = BuildUrl(path);
        ...
        void OnMessage(EventSource eventSource, Message message)
        {
            ...
            string data = JSONNode.Parse(message.Data)["data"].ToString();
    ...
    public void StopListen(string path, Action onClosed = null, Action onFailed = null)
    {
        string url = BuildUrl(path);
        EventSource closing = activeListeners.Find(listener => listener.Uri.OriginalString == url);
        if(closing != null)
        {
            closing.OnClosed += (eventSource) => { Debug.Log($"Listener on {eventSource.Uri} closed successfully"); onClosed?.Invoke(); };
            closing.OnError += (eventSource, error) => { Debug.Log($"Error while closing on {eventSource.Uri}. Error - {error}"); onFailed?.Invoke(); };
            closing.Close();
        }
    }
    ...
    {
        return string.Join("&", parameters.Select(kvp => $"{kvp.Key}={kvp.Value.ToString()}"));
    }
